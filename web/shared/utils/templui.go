...
// SetupScriptRoutes is deliberately absent.
//
// It existed in the templUI *import* workflow, where component JavaScript is
// read out of the module's embedded FS and served from /templui/js. Seshat
// vendors every component into web/shared/ui, so their scripts are ordinary
// files under web/assets/js and are served by the asset handler in cmd/web
// alongside the CSS and fonts — embedded in production, read from disk in
// development. Keeping the import-workflow version would have meant keeping a
// dependency on the templui module purely to feed a function that returns
// immediately (its own guard is componentScriptBasePath != "/templui/js", and
// the CLI rewrote that constant to /assets/js when it vendored the components).
