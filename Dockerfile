...
# The version is pinned rather than tracking latest: a CSS framework that
# changes under a rebuild turns an unrelated deploy into a visual regression.
...
# Build CSS via the same script the Taskfile and CI use, so local, CI and image
...
# without replacing the binary: the migration hook runs `main migrate`.
ENTRYPOINT ["/app/main"]
CMD []
